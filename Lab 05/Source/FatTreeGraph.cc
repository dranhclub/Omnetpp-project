...
int FatTreeGraph::getNumServers() {
    return numServers;
}

int FatTreeGraph::getNumPodSwitches() {
    return numPodSwitches;
}

int FatTreeGraph::getNumCores() {
    return numCores;
}
...
FatTreeGraph::FatTreeGraph(int k){
    ...
    numCores = k * k / 4;
    if (k % 2 == 1)
        throw("K must be even");
    if (k > 256)
        throw("K must less than 256");
